...
contains
  ...
  function interp2d_dp(x,y,array,x0,y0,bounds_error,fill_value) result(value)
    ...
    if(size(x).ne.size(array,1)) call f90wrap_abort("x does not match array")
    if(size(y).ne.size(array,2)) call f90wrap_abort("y does not match array")
  ...
  subroutine check_sort_dp(array)
    ...
    integer :: j
    do j=1,size(array)-1
       if(array(j+1) < array(j)) call f90wrap_abort("Incorrectly sorted array")
    end do
  ...
  subroutine test_quicksort_dp()
    ...
    do j=1,20
       ...
       if(any(array2(index).ne.array1)) call f90wrap_abort("Failure in quicksort_index")
